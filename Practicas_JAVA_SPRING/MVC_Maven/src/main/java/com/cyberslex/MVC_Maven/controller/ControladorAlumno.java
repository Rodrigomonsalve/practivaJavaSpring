...
public class ControladorAlumno {
	...
	modelo.addAttribute("elAlumno", elAlumno);  // En este caso no estamos enviado información a la vista como lo hicimos en ControladorMyLogin, sino que la estamos trayendo. En esta caso de alumnoRegistraFormulario, en donde existe el atributo de etiqueta modelAttribute con el valor "elAlumno".
	...
	public String procesaFormulario(@Valid @ModelAttribute("elAlumno") Alumno elAlumno, BindingResult resultadoValidacion) {  //Funciona muy parecido a la anotación @RequestParam que utiliamos en la clase ControladorMyLogin. El objeto elAlumno de este método es diferente al del método pasado. Lo que el usuario puso en la vista (alumnoRegistroFormulario) viaja a través del identificador "elAlumno" y se guarda en el objeto elAlumno.
