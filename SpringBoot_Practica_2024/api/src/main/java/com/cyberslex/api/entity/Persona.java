...
//	private Long id;
	...
	@JoinColumn(name="fk_domicilio")  // Con la anotación JoinColumn puedes crear una llave foránea. La regla de mapeo dice que, tratándose de relaciones 1:1, la llave primaria de una las tablas, en este caso de "Domicilio", pasará a la otra tabla como llave foránea. En nuestro caso, la columna de llave foránea en Persona llevará por nombre "fk_domicilio". Estamos creando una nueva columna. Esto sucede si la combinamos con @OneToOne y @ManyToOne. Sin embargo, en el caso de la anotación @OneToMany sucede lo contrario: al combinarla con la anotación @JoinColumn la clave foránea se pasa a la entidad sobre la cual se escriba, en nuestro caso Domicilio.
